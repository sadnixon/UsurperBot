...
target_fps = 60
title = "Usurper: Dark Factions     FPS"
...
def input_waiter_xy(game_width, game_height, game_surface, screen, setup, card_selection_index, player=True):
    ...
    while (not (card_placement_x in range(3) and card_placement_y in range(3))) or [setup.p_zero_grid, setup.p_one_grid][setup.turn][card_placement_x][card_placement_y] != 0 or [setup.p_zero_hand, setup.p_one_hand][setup.turn][card_selection_index].placement_grid[card_placement_x][card_placement_y] == 0:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                setup.set_card_sizes(
                    event.size[0], event.size[1], game_width, game_height)
            elif event.type == KEYDOWN:
                if int(event.key) in keys_to_coords:
                    card_placement_x, card_placement_y = keys_to_coords[int(
                        event.key)]
                ...
                card_placement_y = math.floor(
                    ((mouse_x-left_cards)/cards_divisor)*3)
        ...
        pygame.transform.smoothscale(
            game_surface, (max_surface_x, max_surface_y), screen_surface)
...
def input_waiter_play(screen, setup, game_width, game_height, player=True):
    ...
    img_list, rect_list, topleft_list, placement_rect_list = renderGameNext2(
    ...
    while (not (card_placement_x in range(3) and card_placement_y in range(3))) or [setup.p_zero_grid, setup.p_one_grid][setup.turn][card_placement_x][card_placement_y] != 0 or [setup.p_zero_hand, setup.p_one_hand][setup.turn][card_selection_index].placement_grid[card_placement_x][card_placement_y] == 0:
        ...
        renderGameNext2_update(screen, game_width, game_height,
                               setup, img_list, rect_list, topleft_list, moving_index)
...
def input_waiter_yesno(screen, setup, game_width, game_height, instant_id, highlight_x=-1, highlight_y=-1):
    ...
    yes_rect, no_rect = renderYesNo(
        screen, game_width, game_height, instant_id, highlight_x, highlight_y, mouse_over)
    ...
    while decision == 'n/a':
        ...
        renderYesNo(screen, game_width, game_height, instant_id,
                    highlight_x, highlight_y, mouse_over)
...
def input_waiter_g7(screen, setup, game_width, game_height, player=True):
    ...
    img_list, rect_list, topleft_list = renderGameG7(
        screen, game_width, game_height, setup)
    ...
    while (not (move_x in range(3) and move_y in range(3) and target_x in range(3) and target_y in range(3))):
        ...
        renderGameG7_update(screen, game_width, game_height,
                            setup, img_list, rect_list, topleft_list, moving_index)
...
def input_waiter_draw_discard(screen, setup, game_width, game_height, instant_id):
    ...
    img_list, rect_list, topleft_list, placement_rect = renderGameDiscardDraw(
        screen, game_width, game_height, setup, instant_id)
    ...
    while (instant_id not in ['R2', 'R7'] and card_selection_x not in range(range_len)) or (instant_id in ['R2', 'R7'] and ((not (card_selection_x in range(3) and card_selection_y in range(3))) or [setup.p_zero_grid, setup.p_one_grid][setup.turn][card_selection_x][card_selection_y] == 0)):
        ...
        renderGameDiscardDraw_update(screen, game_width, game_height, setup,
                                     instant_id, img_list, rect_list, topleft_list, moving_index)
...
def input_waiter_flip(screen, setup, game_width, game_height, instant_id):
    ...
    if instant_id == 'R4':
        ...
        relevant_grid = [setup.p_zero_grid,
                         setup.p_one_grid][abs(1-setup.turn)]

    placement_rect_list = renderGameFlip(
        screen, game_width, game_height, setup, instant_id)
    ...
    while (not (card_flip_x in range(3) and card_flip_y in range(3))):
        ...
        renderGameFlip(screen, game_width, game_height,
                       setup, instant_id, highlight)
...
def input_waiter_results(screen, setup, game_width, game_height, p_zero_name, p_one_name, p_zero_score, p_one_score, p_zero_bonus_score, p_one_bonus_score):
    ...
    yes_rect, no_rect = renderGameResults(screen, game_width, game_height, setup, p_zero_name,
                                          p_one_name, p_zero_score, p_one_score, p_zero_bonus_score, p_one_bonus_score)
